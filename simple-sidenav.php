...
Plugin URI: http://simplerealtytheme.com/plugins/
...
Author: Max Chirkov
Version: 2.3.0
Author URI: http://www.SimpleRealtyTheme.com
*/
...
function simple_sidenav_pages($args = '') {
	...
	;
		...
		if ( isset($current_page) && get_permalink($current_page) == $url ) {
			...
			for ($i=0; $i<count($link); $i++ ) {
				//$li_class_current_page = ' class="' . simple_sidenav_current_page($link[$i]['title']) . '"';
					...
					if(array_key_exists('page', $link[$i])){
						...
						if(ssn_get_option('target_attr') && $link[$i]['target']){
...
class simple_sidenav extends \WP_Widget
{
    function __construct()
    {
		...
		/* Create the widget. */
		parent::__construct( 'simple_sidenav', 'Simple Sidebar Navigation', $widget_ops, $control_ops );
    }
...
//		$action = "1";
//		$show = "all";
		 $number = (isset($widget_id)) ? $widget_id : '';
...
{
	$custom_css = ssn_get_option('custom_css');
